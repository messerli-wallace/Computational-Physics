...
!
      ...
      real*8 inverse_home,inverse_lapack

      real*8, allocatable :: a(:,:),b(:,:),c(:,:),d(:,:),e(:,:)
      real*8, allocatable :: work(:),eigens(:)
      ...
      write(1,*) "ndim,       inverse_home,     inverse_lapack,   diag_home,  diag_lapack"
      do ndim = 500,5000,100
            print*,"ndim = ",ndim
            allocate(a(ndim,ndim),b(ndim,ndim),c(ndim,ndim),&
            d(ndim,ndim),e(ndim,ndim),ipiv(ndim),eigens(ndim))
            ...
            ! ! enddo
            ! !endif
            ! inverse_home = end_1 - start_1
            ! print*, "Time = ",inverse_home, "seconds"
            print*, 'Inverse of Matrix A using LAPACK:'
            d=a
            call cpu_time(start_2)
            !call DSYTRI('U',ndim,c,ndim,ipiv,work,info)
            allocate(work(ndim))
            call DGETRI(ndim,a,ndim,ipiv,work,-1,info)
            print*,info
            lwork = work(1)
            print*,lwork
            deallocate(work)
            allocate(work(lwork))
            call DGETRI(ndim,a,ndim,ipiv,work,lwork,info)
            print*,info
            ! print*,info
            deallocate(work)
            call cpu_time(end_2)
            inverse_lapack = end_2 - start_2
            print*, "Time = ",inverse_lapack, "seconds"
            print*
            ! call matmup(a,d,e,ndim,ndim,ndim)
            ! print *
            if (ndim.le.10) then
                  do i=1,ndim
                   write(6,1000) (a(i,j),j=1,ndim)
      ...
      !       call cpu_time(end_3)
      !       d_h = end_3-start_3
      !       print*, "Time = ",d_h," seconds"

            ! print*, "Diagonalization time for LAPACK:"
            ! allocate(work(ndim))
            ! CALL DSYEV('V','U',ndim,A,ndim,eigens,work,-1,info)
            ! lwork = work(1)
            ! deallocate(work)
            ! allocate(work(lwork))
            ! call cpu_time(start_4)
            ! CALL DSYEV('V','U',ndim,A,ndim,eigens,work,lwork,info)
            ! call cpu_time(end_4)
            ! deallocate(work)
            ! d_lapack = end_4-start_4
            ! print*, "Time = ",d_lapack," seconds"
            
            write(1,*) ndim, inverse_home, inverse_lapack, d_h, d_lapack
...
!*
      real*8 AMATL(ICOROW,ICOLR),AMATR(ICOROW,ICOLR), &
         AMULT(ICOROW,ICOLR)
...
!*      
12    TANG=SIGN(TWO,(H(IPIV,IPIV)-H(JPIV,JPIV)))*H(IPIV,JPIV)/ (ABS(H(IPIV,IPIV)&
-H(JPIV,JPIV))+SQRT((H(IPIV,IPIV) -H(JPIV,JPIV))**2 &              
     +FOUR*H(IPIV,JPIV)**2)) 
